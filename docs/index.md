...
- `password` (String, Sensitive) The service account password. When both `service_account` and `password` are omitted, the provider automatically uses the OAuth2 Device Authorization Flow (RFC 8628) for interactive login.
- `service_account` (String) The service account username. When both `service_account` and `password` are omitted, the provider automatically uses the OAuth2 Device Authorization Flow (RFC 8628) for interactive login.
