...
Values in `data_wo` are write-only: they're transmitted to the server but never stored in state or displayed in plans. This is the recommended approach for highly sensitive credentials like API keys or temporary tokens.
