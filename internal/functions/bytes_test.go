...
func TestBytesFunctionRun(t *testing.T) {
	testCases := map[string]struct {
		...
		"32-bytes": {
			...
			expected: function.RunResponse{
				Result: function.NewResultData(types.Int64Value(32)), // rounded up
			},
		},
		"2-megabytes-in-bytes": {
			...
			expected: function.RunResponse{
				Result: function.NewResultData(types.Int64Value(2000000)), // rounded up, as 2,000,000 bytes is ~1.907 MiB
			},
		},
		"1-gibibyte-in-bytes": {
			request: function.RunRequest{
			...
			expected: function.RunResponse{
				Result: function.NewResultData(types.Int64Value(1073741824)),
			},
		},
		"32-gigabytes-in-bytes": {
			...
			expected: function.RunResponse{
				Result: function.NewResultData(types.Int64Value(32000000000)),
			},
		},
		"1Gi": {
			request: function.RunRequest{
		...
		"1.1Gi": {
			request: function.RunRequest{
				Arguments: function.NewArgumentsData([]attr.Value{types.DynamicValue(types.StringValue("1.1Gi"))}),
			},
			expected: function.RunResponse{
				Result: function.NewResultData(types.Int64Value(1181116007)),
			},
		},
		"1234Mi": {
			request: function.RunRequest{
			...
			expected: function.RunResponse{
				Result: function.NewResultData(types.Int64Value(1293942784)),
			},
		},
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			BytesFunction{}.Run(context.Background(), testCase.request, &got)
